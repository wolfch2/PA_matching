...
```{r}
library(aws.s3)
save_from_s3(from = "Replication_wolf/WDPA_Jan2023_Public_shp.zip",
             to = "temp/WDPA_Jan2023_Public_shp.zip")
```
```{r}
library(tidyverse)
library(sf)
...
# Delete files
file.remove(list.files("temp", recursive = TRUE, 
                       full.names = TRUE))
# Delete folders
list.dirs("temp", recursive = TRUE, full.names = TRUE) %>%
  str_subset("temp$", negate = TRUE) %>%
  map(unlink, recursive = TRUE)
# Write WDPA
st_write(obj = wdpa, 
...
```{r}
library(aws.s3)
my_wdpa_shp <- list.files(path = "data_input/PAs",
                     pattern = "WDPA_Jan2023",
                     full.names = TRUE)
my_wdpa_shp
my_wdpa_shp_s3 <- paste0("Replication_wolf/", my_wdpa_shp)

map2(my_wdpa_shp, my_wdpa_shp_s3, put_to_s3)
```
